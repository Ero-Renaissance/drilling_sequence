...
class RigContractUpsert(BaseModel):
    ...
    A contract IS its end date — `contract_end` is required (a date-less
    contract carries no decision-driving information; removing a contract is
    the DELETE endpoint, not a blanked-out upsert). `terrain` names WHICH
    physical rig when a name exists in more than one terrain (e.g. a LAND and
    a SWAMP "10K Rig 1"). Omitted, the server resolves it from the registry
    when unambiguous and 409s when it isn't — so existing clients keep working
    for every single-terrain rig."""
