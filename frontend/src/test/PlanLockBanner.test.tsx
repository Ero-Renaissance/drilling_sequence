...
describe("PlanLockBanner", () => {
  it("offers Revise Plan for an approved (frozen) campaign", () => {
    render(<PlanLockBanner projectId="p1" lock={lock("approved")} />);
  ...
  it("is informational (no action) while pending approval", () => {
    render(<PlanLockBanner projectId="p1" lock={lock("pending")} />);
  ...
  it("renders nothing when the plan is not locked", () => {
    const { container } = render(
      <PlanLockBanner projectId="p1" lock={{ ...lock("approved"), locked: false }} />,
    );
